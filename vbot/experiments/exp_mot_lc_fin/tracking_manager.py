...
class TrackingManager:
    ...
    def display(self):
        ...
        # collect all params
        ellipse_params = self.get_ellipse_params(IMAGE_REF_FRAME)
        ellipse_semi_axes = ellipse_params[:2]
        ellipse_center = ellipse_params[2]
        ellipse_rotation_angle = ellipse_params[3]  # radians
        
        
        ellipse_focal_point_1 = ellipse_params[4]
        ellipse_focal_point_2 = ellipse_params[5]
        ...
        ellipse_focal_point_1_est = self.convert(ellipse_focal_point_1_est)
        ellipse_focal_point_2_est = self.convert(ellipse_focal_point_2_est)
        ellipse_focal_point_1_est = ellipse_focal_point_1_est
        ellipse_focal_point_2_est = ellipse_focal_point_2_est
        ...
        # compute xmax, xmin
        x1 = h + ellipse_semi_axes[0] * cos(ellipse_rotation_angle) * cos(t_x%tau)    - ellipse_semi_axes[1] * sin(ellipse_rotation_angle) * sin(t_x%tau)
        x2 = h + ellipse_semi_axes[0] * cos(ellipse_rotation_angle) * cos((t_x+pi)%tau) - ellipse_semi_axes[1] * sin(ellipse_rotation_angle) * sin((t_x+pi)%tau)
        x_max, x_min = (x1, x2) if x1 > x2 else (x2, x1)

        # compute ymax, ymin
        y1 = k + ellipse_semi_axes[0] * sin(ellipse_rotation_angle) * cos(t_y%tau)    + ellipse_semi_axes[1] * cos(ellipse_rotation_angle) * sin(t_y%tau)
        y2 = k + ellipse_semi_axes[0] * sin(ellipse_rotation_angle) * cos((t_y+pi)%tau) + ellipse_semi_axes[1] * cos(ellipse_rotation_angle) * sin((t_y+pi)%tau)
        y_max, y_min = (y1, y2) if y1 > y2 else (y2, y1)
    ...
    def compute_focal_point_estimations(self):
        ...
        fpmid_x, fpmid_y = self.ellipse_params_meas[2]
        major_axis = self.ellipse_params_meas[0]

        # filter focal points
        self.ellipse.EKF.add(fp1_x, fp1_y, fp2_x, fp2_y, major_axis, fpmid_x, fpmid_y)
